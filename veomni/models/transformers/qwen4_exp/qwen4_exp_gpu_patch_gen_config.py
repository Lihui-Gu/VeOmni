...
config.add_import("veomni.ops.kernels.qsa_attention", names=["qsa_attention_eager"])
...
config.add_post_import_block(
    ...
    # uses the compact PyTorch implementation for eager QSA. GatedDeltaNet
...
# Use local-query/global-block compact selection without constructing a dense
# selected-token mask.
# ================================================================
@config.override_method(
...
# 3. Run eager QSA over compact indices without constructing a dense mask.
# ================================================================
@config.override_method(
    "Qwen4ExpTextAttention.forward",
    description="Run compact eager QSA with global selection and Ulysses QKV exchange",
)
def qwen4_exp_attention_forward_patched(
    self,
    hidden_states: torch.Tensor,
    position_embeddings: tuple[torch.Tensor, torch.Tensor],
    attention_mask: torch.Tensor | None,
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_output = qsa_attention_eager(
        query_states,
        key_states,
        value_states,
        selection,
...
# 1. Skip the quadratic QSA mask for compact-index backends while retaining
